...
namespace SignageBackend.Controllers
{
    ...
    {
        private readonly IGroupSchedule _groupSchedule;
        private int ResID;

        public GroupScheduleController(IGroupSchedule groupSchedule)
        {
            _groupSchedule = groupSchedule;
        }

        [HttpPost("CreateOrUpdate")]
        [ProducesResponseType((int)HttpStatusCode.BadRequest)]
        [ProducesResponseType((int)HttpStatusCode.Created)]
        public IActionResult CreateOrUpdate([FromBody] GroupScheduleView groupScheduleView )
        {
            try
            {
                ...
                {
                    ResID = _groupSchedule.CreateOrUpdate(groupScheduleView);
                    if (ResID != 0)
                    {
                        if (groupScheduleView.GroupScheduleId == 0)
                        {
        ...
        {
            var result = await _groupSchedule.GetById(id);
            if (result == null)
            {
        ...
        {
            var result = await _groupSchedule.GetAll(searchTerm, sortBy, sortDescending, pageNumber, pageSize);
            if (result == null)
            {
                return Ok(new ApiResponse { Status = "No Record Found", Code = 200 });
            }
        ...
        {
            var result = await _groupSchedule.DeleteById(id);
            if (result == null)
            {
